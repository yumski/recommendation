...
The data is split into 6 columns consist of userId, movieId, date, rating and genre.
...
```
With this small sample we can already start seeing clusters of users.
```{r message=FALSE, warning=FALSE}
heatmap(as.matrix(d), col = RColorBrewer::brewer.pal(11, "Spectral"))
```
We can also visualize this better with a heatmap.
...
The graph for movie bias is consistent with our findings in the movie distribution graph. Majority of the movies are rated very close to the average, which some exceptional movies receiving a high rating. The $$b_i$$ is skewed towards negative, meaning majority of the movies are rated below the average.
...
The distribution of the user bias is similar to the one for the movies. However, unlike the the movie bias, the $$b_u$$ is a balanced distribution. The majority of the users rate very close to the $$\mu$$.
```{r message=FALSE, warning=FALSE}
...
To account for the low number of rating, we introduce $$\lambda$$ to penalize the regression to control the variability. This method will shrink the $$\hat{b}_i$$ towards 0 when the $$n_i$$ is low, but will be effectively ignored when it has a lot of rating.

$$lambda$$ is a tuning parameter, we can use machine learning to determine what's the optimal number.
```{r message=FALSE, warning=FALSE}
#define lambda range
...
$$\lambda$$ was applied to the algorithm at an interval of 0.25 from 0 to 10. We see that 5.0 provides the lowest RMSE.
...
Overall this project was challenging and fun to work on. I got to apply the knowledge I learned from the course and built upon the examples from Professor Irizarry. Working with such a large data set did come with some challenges and limitations. I was not able to run some models due to hardware limitation, excited to experiment on another data set for other class project.
